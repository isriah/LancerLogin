...
- **resume [selector | complete-all]**: Reconcile a previously authorized run before taking any new action. Inspect every recorded task ID, provisioning ID, branch, and candidate Worktree for the selected `in progress` units. Do not create a replacement, reserve another unit, or start a later cohort until each selected unit is confirmed active, has been resolved by the provisioning deadline, or is recorded `blocked`/`failed`. If an unowned candidate Worktree must be removed to allow a replacement, show its exact path and request current user approval before deleting it. With `complete-all`, continue the most recently recorded execution set after reconciliation; do not add units that became ready after that run began.
...
For each cohort, first create the named `codex/wu-…` branches from the committed reservation base and verify their refs. Then provision one task at a time from its named branch. A returned `clientThreadId` is a pending Worktree-provisioning result: retain it and make at most three bounded, event-driven checks over three minutes for a task ID and non-detached branch-owned Worktree before provisioning the next task. Only after every task in the cohort is confirmed may their implementations run in parallel. Do not repeatedly poll task lists or worktree state unless a provisioning timeout, completion event, or a specific integration question requires inspection. At the deadline, inspect the named branch: integrate a clean implementation commit beyond the recorded base as recovered completion evidence; otherwise record the unit `failed` with the provisioning evidence and continue unrelated selected work. A detached candidate Worktree or candidate without a resolved task ID is not a valid implementation environment and cannot be reused or removed without confirming no task owns it and obtaining current user approval.
