---
name: lancerlogin-work-unit
description: "Implement one selected LancerLogin work unit in an isolated Worktree, with its documented sources, verification, and commit handoff. Use after a coordinator reserves a work unit; do not use to coordinate or integrate parallel work."
---
...
Use this skill only for `implement <WU-ID>` in a Worktree task reserved by the coordinator. Confirm the task is in a Worktree and on the branch recorded for the unit; create the named `codex/wu-...` branch if necessary. Rename the task to `<WU-ID> — <work-unit title>`.
