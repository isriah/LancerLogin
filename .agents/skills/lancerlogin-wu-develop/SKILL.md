...
For a parallel branch, do not edit `docs/future_work.md` or merge. Report the commit SHA, changed files, verification, and integration risks for `$lancerlogin-integrate`. For a serial local change that the user authorized on `main`, update the WU to `merged` only after the commit is safely on `main` and record its release impact.
