...
- **integrate <WU-ID>**: Outside a multi-WU goal coordinator, invoke `$ll-integrate <WU-ID>` and follow that skill. A goal coordinator creates a dedicated integration task instead.

In a multi-WU goal task, remain orchestration-only. Create one dedicated Worktree task for every WU, including units that must run serially. Use bounded task-status and wait summaries, ask product decisions in the goal task, and create a separate integration task for eligible named candidates and a separate release task for an explicitly authorized bundle. Never rename the coordinator after a WU, invoke `$ll-wu-develop` in it, edit WU code or tests, run WU verification, integrate, or release there.
