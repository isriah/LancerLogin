...
- **integrate all**: Read-only. Inspect every `in progress` WU for a recorded branch with a reviewable commit beyond its base and completed handoff evidence. Propose the safe serial merge order, identify units that are not ready, and state the focused verification for each proposed merge.
- **integrate suggested**: Treat this as approval of the serial merge batch proposed by the immediately preceding `integrate all` response in this same task. Re-read the ledger and revalidate each branch before merging. Integrate one branch at a time using the `$ll-integrate` procedure, including its focused verification and ledger update. Stop at the first missing evidence, conflict, or failed verification; preserve remaining branches and report the exact next action. Do not release, deploy, or start new implementation tasks.
