...
- Provision tasks serially even when their implementations will run in parallel. A returned `clientThreadId` means Worktree setup is still pending, not that creation failed: record it, wait for a resolved task ID and branch-owned Worktree or an explicit provisioning failure, then provision the next task. Start parallel implementation only after every task in the cohort is confirmed.
