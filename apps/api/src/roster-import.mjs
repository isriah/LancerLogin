...
export function parseRosterCsv(input) {
  ...
  const seen = new Set(); const rows = []; const errors = [];
  for (let index = 1; index < lines.length; index += 1) {
    ...
    rows.push({ memberId: row.memberId, firstName: row.firstName, lastName: row.lastName, email: row.email || undefined, discordUserId: row.discordUserId || undefined });
  }
