...
export function AppShell({ role, branding, onBrandingChanged, onSignedOut }: { role: "admin" | "operator"; branding: Branding; onBrandingChanged: (branding: Branding) => void; onSignedOut: () => void }) {
  const { path, search, navigate } = usePath(); const [setupKey, setSetupKey] = useState(0); const [onboarding, setOnboarding] = useState(role === "admin" ? undefined as boolean | undefined : false); const [mobileNavOpen, setMobileNavOpen] = useState(false); const [integrations, setIntegrations] = useState<IntegrationCapabilities>(disabledCapabilities);
