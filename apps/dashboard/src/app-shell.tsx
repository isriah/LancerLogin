...
export function AppShell({ role, branding, onBrandingChanged, onSignedOut }: { role: "admin" | "operator"; branding: Branding; onBrandingChanged: (branding: Branding) => void; onSignedOut: () => void }) {
  ...
  else if (path === "/kiosks") page = <KiosksPage role={role} openSetup={() => setOnboarding(true)} />;
  else if (role === "admin" && path === "/settings/organization") page = <OrganizationSettings initialBranding={branding} onChanged={onBrandingChanged} onOpenSetup={() => setOnboarding(true)} />;
  ...
  else if (role === "admin" && path === "/settings/data") page = <DataSettings onReset={() => { setSetupKey((value) => value + 1); setOnboarding(true); }} />;
