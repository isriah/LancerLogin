...
export function AttendanceWorkspace({ meetingId, role }: { meetingId: string; role: "admin" | "operator" }) {
  ...
  return <section className="attendance-workspace meeting-detail-workspace" aria-labelledby="meeting-detail-title" data-walkthrough-page="meeting" data-walkthrough-ready={meeting ? "true" : undefined} tabIndex={-1}>
    <nav className="meeting-detail-navigation" aria-label="Meeting detail navigation" data-walkthrough="meeting-navigation">
    ...
    {meeting ? <>
      ...
      {(calendarProviders.length > 0 || discordConfigured) && <section className="task-card meeting-actions-card ui-card" aria-labelledby="meeting-actions-title" data-walkthrough="meeting-integrations"><div className="panel-heading"><InfoHeading id="meeting-actions-title" info="Run calendar delivery and Discord operations for this meeting.">Meeting actions</InfoHeading></div><div className="meeting-actions-buttons">{calendarProviders.length > 0 && <button className="primary-button" type="button" disabled={calendarBusy} onClick={() => void syncCalendars()}>{calendarBusy ? "Syncing calendars…" : "Sync configured calendars"}</button>}{discordConfigured && <button className="ui-button" type="button" disabled={!absenceEligible || Boolean(discordBusy)} onClick={() => void notifyDiscordAbsences()}>{discordBusy === "absence" ? "Sending…" : "Send Discord absence notice"}</button>}</div>{calendarNotice && <p className="meeting-discord-notice ui-status" data-tone={calendarNoticeTone} role="status" aria-live="polite">{calendarNotice}</p>}{discordNotice && <p className="meeting-discord-notice ui-status" data-tone={discordNoticeTone} role="status" aria-live="polite">{discordNotice}</p>}</section>}
      {discordConfigured && contests.length > 0 && <section className="task-card meeting-contests ui-card" aria-labelledby="meeting-contests-title" data-walkthrough="meeting-contests"><div className="panel-heading"><InfoHeading id="meeting-contests-title" info="Review requests submitted for this meeting.">Attendance contests</InfoHeading><span className="progress-count">{contests.length} open</span></div><ContestReviewList contests={contests} onResolved={(resolution, contest) => { setContests((current) => current.filter((item) => item.meetingId !== contest.meetingId || item.memberId !== contest.memberId)); setDiscordNotice(`Contest ${resolution}.`); setDiscordNoticeTone("success"); if (resolution === "approved") void refreshAttendance(meeting.id).catch((error: Error) => setNotice(error.message)); }} /></section>}
