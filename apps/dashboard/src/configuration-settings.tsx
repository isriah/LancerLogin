...
export function ConfigurationSettings({ initialBranding, onChanged }: { initialBranding: Branding; onChanged: (branding: Branding) => void }) {
  ...
  async function save(event: FormEvent) {
    event.preventDefault(); const next = { ...initialBranding, lateScanMinutes: lateScanMinutes.trim() === "" ? 0 : Number(lateScanMinutes), discordContestWindowHours: discordContestWindowHours.trim() === "" ? 24 : Number(discordContestWindowHours), attendanceReportingStartsOn: attendanceReportingStartsOn || null };
