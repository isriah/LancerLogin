...
export function ContestIndicator({ enabled }: { enabled: boolean }) {
  const [contests, setContests] = useState<Contest[]>([]); const [open, setOpen] = useState(false); const [notice, setNotice] = useState(""); const dialog = useRef<HTMLElement>(null);
  const load = useCallback(async () => { const result = await api<{ contests: Contest[] }>("/discord/contests"); setContests(result.contests.filter((contest) => contest.status === "open")); }, []);
  function close() { setOpen(false); setNotice(""); }
  useEffect(() => {
    if (!enabled) { setContests([]); setOpen(false); setNotice(""); return; }
  ...
  if (!enabled || (!contests.length && !open && !notice)) return null;
