...
export function ContestIndicator({ enabled }: { enabled: boolean }) {
  ...
  return <>{count > 0 && <button className="contest-indicator" data-count={count} type="button" aria-haspopup="dialog" aria-expanded={open} aria-controls="contest-review-dialog" onClick={() => { setNotice(""); setOpen(true); }} aria-label={`${count} attendance contest${count === 1 ? "" : "s"} awaiting review. Open contest review.`}><span aria-hidden="true">!</span> {count} contest{count === 1 ? "" : "s"} awaiting review</button>}{open && <div className="dialog-backdrop"><section id="contest-review-dialog" className="ui-dialog contest-review-dialog" role="dialog" aria-modal="true" aria-labelledby="contest-review-title" tabIndex={-1} ref={dialog}><div className="dialog-heading"><div><h2 id="contest-review-title">Contests awaiting review</h2><p>Review pending attendance contests without leaving your current page.</p></div><button type="button" aria-label="Close contest review dialog" onClick={close}>×</button></div>{notice && <p className="ui-status setup-status" role="status">{notice}</p>}{count ? <ContestReviewList contests={contests} onResolved={(resolution) => setNotice(`Contest ${resolution}.`)} /> : <p className="empty-state">No attendance contests need review.</p>}</section></div>}</>;
}
