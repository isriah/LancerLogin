...
function DataActionDialog({ dialog, file, confirmation, error, busy, onClose, onFile, onConfirmation, onRestore, onDelete }: { dialog: Dialog; file?: File; confirmation: string; error: string; busy: boolean; onClose: () => void; onFile: (file?: File) => void; onConfirmation: (value: string) => void; onRestore: () => void; onDelete: () => void }) {
  const modal = useRef<HTMLElement>(null); const detail = details[dialog.scope]; const restoring = dialog.action === "restore"; const typedConfirmation = restoring ? `RESTORE ${dialog.scope.toUpperCase()}` : detail.confirmation; const actionReady = confirmation.trim() === typedConfirmation && (!restoring || Boolean(file));
  useModalFocus(modal, true, busy, onClose);
  return <div className="dialog-backdrop"><section className="data-action-dialog" role="dialog" aria-modal="true" aria-labelledby="data-action-title" aria-describedby="data-action-description" tabIndex={-1} ref={modal}><div className="dialog-heading"><div><h2 id="data-action-title">{restoring ? `Restore ${detail.title}` : `Delete ${detail.title}`}</h2><p id="data-action-description">{restoring ? detail.restore : detail.deletion}</p></div><button type="button" aria-label="Close data action dialog" disabled={busy} onClick={onClose}>×</button></div>{restoring ? <label className="file-picker">Backup file<input type="file" accept="application/json,.json" onChange={(event) => { onFile(event.target.files?.[0]); }} /><span>{file ? file.name : "Choose a JSON backup file"}</span></label> : <p className="warning-callout" role="alert">This action permanently deletes the selected category. Review the scope and download a backup first if you may need it.</p>}<label>Type <code>{typedConfirmation}</code> to confirm<input autoComplete="off" value={confirmation} aria-invalid={Boolean(error)} onChange={(event) => onConfirmation(event.target.value)} aria-describedby={error ? "data-action-error" : undefined} /></label>{error && <p id="data-action-error" className="inline-messages error" role="alert" tabIndex={-1}>{error}</p>}<div className="dialog-actions"><button type="button" disabled={busy} onClick={onClose}>Cancel</button><button className={restoring ? "primary-button" : "danger-button"} type="button" disabled={busy || !actionReady} onClick={restoring ? onRestore : onDelete}>{busy ? (restoring ? "Restoring…" : "Deleting…") : (restoring ? "Validate and restore" : `Delete ${detail.title.toLowerCase()}`)}</button></div></section></div>;
}
