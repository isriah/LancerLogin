...
export function HomePage({ navigate, discordEnabled }: { role: "admin" | "operator"; navigate: (path: string) => void; discordEnabled: boolean }) {
  ...
  async function load() { const result = await api<{ meetings: Meeting[] }>("/meetings"); setMeetings(result.meetings); setNotice(""); setNoticeTone("neutral"); }
