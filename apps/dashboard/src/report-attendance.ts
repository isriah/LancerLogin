...
export type ReportHistory = { ruleId: string; labelId: string; labelName: string; ruleType: "weighted_percentage" | "weekly_count"; excusedHandling: "exclude" | "count_missed" | null; startsOn: string | null; endsOn: string | null; rate: number | null; unadjustedRate: number | null; weeksMet: number; weeksDue: number; status: string };
export type ReportMember = { member: { id: string; memberId: string; firstName: string; lastName: string; active: number | boolean }; currentLabelIds: string[]; rows: ReportRow[]; regularAttendance: RegularAttendance; policy: "standard" | "weekly" | "mixed"; present: number; primaryTotal: number; adjustedTotal: number; rate: number | null; adjustedRate: number | null; pooledRate: number | null; weeks: ReportWeek[]; belowTargetWeeks: ReportWeek[]; currentCompliance: ReportCompliance; historySummaries: ReportHistory[] };
...
export function defaultRosterRates(report: AttendanceReport) { return { meetingCount: report.meetings.length, rates: Object.fromEntries(report.members.map((item) => [item.member.id, { regularAttendance: item.regularAttendance, currentCompliance: item.currentCompliance, complete: true }])) }; }
