...
export function RosterImportPanel({ members, onImported, open, onClose }: Props) {
  ...
  function prepare() {
    ...
    const seen = new Set<string>(); const values = parsed.map((row) => Object.fromEntries(header.map((name, column) => [name, row[column] ?? ""])) as IncomingMember).filter((member, index) => {
