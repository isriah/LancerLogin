...
export function usePath() {
  ...
  useEffect(() => { if (window.location.pathname.replace(/\/+$/, "") === "/settings/privacy") window.history.replaceState({}, "", `${normalizePath(window.location.pathname)}${window.location.search}`); const update = () => setLocation({ path: normalizePath(window.location.pathname), search: window.location.search }); window.addEventListener("popstate", update); window.addEventListener(routeChangeEvent, update); return () => { window.removeEventListener("popstate", update); window.removeEventListener(routeChangeEvent, update); }; }, []);
