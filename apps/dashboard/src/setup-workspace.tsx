...
export function SetupWorkspace({ initialBranding, onBrandingChanged, onSignedOut, embedded = false, onComplete }: { initialBranding: Branding; onBrandingChanged: (branding: Branding) => void; onSignedOut?: () => void; embedded?: boolean; onComplete?: () => void }) {
  ...
  const [testMeetingStarts, setTestMeetingStarts] = useState(new Date(Date.now() - 5 * 60_000).toISOString().slice(0, 16));
  const [testMeetingEnds, setTestMeetingEnds] = useState(new Date(Date.now() + 55 * 60_000).toISOString().slice(0, 16));
  ...
  async function createTestMeeting(event: FormEvent) {
    ...
    try {
      const result = await api<{ meeting: Meeting }>("/meetings", { method: "POST", body: JSON.stringify({ title: testMeetingTitle, startsAt: new Date(testMeetingStarts).toISOString(), endsAt: new Date(testMeetingEnds).toISOString(), required: true, isTest: true, notes: "Created by guided setup; simulated attendance is allowed only in test meetings." }) });
