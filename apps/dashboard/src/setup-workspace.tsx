...
import { MeetingCreationDialog, type Meeting } from "./meetings-page";
import type { CalendarSync, CalendarDelivery } from "./meeting-management";
...
export type Branding = { organizationName: string; subtitle?: string; logoData?: string; primaryColor: string; secondaryColor: string; appearance: "system" | "light" | "dark"; logoBackdrop: LogoBackdrop; lateScanMinutes: number; discordContestWindowHours: number; attendanceReportingStartsOn?: string | null; anomalyLateThresholdMinutes?: number; anomalyEarlyThresholdMinutes?: number };
...
const setupMeetingKey = "lancerlogin-setup-meeting";
...
export function SetupWorkspace({ initialBranding, onBrandingChanged, onSignedOut, embedded = false, onComplete }: { initialBranding: Branding; onBrandingChanged: (branding: Branding) => void; onSignedOut?: () => void; embedded?: boolean; onComplete?: () => void }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setMeetings(meetingStatus.meetings);
      let savedMeetingId: string | null = null;
      try { savedMeetingId = sessionStorage.getItem(setupMeetingKey); } catch { /* Selection still works when browser storage is unavailable. */ }
      selectTestMeeting(meetingStatus.meetings.find((meeting) => meeting.id === savedMeetingId)?.id ?? meetingStatus.meetings[0]?.id ?? "");
  ...
  function selectTestMeeting(meetingId: string) {
    setTestMeetingId(meetingId); setSimulatedArrivalSent(false); setAttendanceRows([]);
    try { if (meetingId) sessionStorage.setItem(setupMeetingKey, meetingId); else sessionStorage.removeItem(setupMeetingKey); } catch { /* Storage is optional; API attendance remains authoritative. */ }
  }
  async function meetingCreated(_count: number, _calendarSync?: CalendarSync, _calendarDelivery?: CalendarDelivery, createdMeetingIds: string[] = []) {
    let result: { meetings: Meeting[] };
    try { result = await api<{ meetings: Meeting[] }>("/meetings"); }
    catch {
      showMessages(currentStep, { errors: ["Meeting creation succeeded, but the schedule could not be refreshed. Reload guided setup before creating another meeting."] });
      return;
    }
    const created = result.meetings.filter((meeting) => createdMeetingIds.includes(meeting.id)).sort((left, right) => Date.parse(left.startsAt) - Date.parse(right.startsAt))[0];
    setMeetings(result.meetings);
    if (created) selectTestMeeting(created.id);
    showMessages(currentStep, {}); setNotice("Meeting created. Setup remains incomplete until scans and attendance are verified.");
  }
  ...
  const selectedMeeting = setupMeetings.find((meeting) => meeting.id === testMeetingId);
  const meetingChoice = <div>
    {setupMeetings.length === 0 && <p>No meetings are available yet. Create your first meeting here to test attendance.</p>}
    <label>Meeting<select value={testMeetingId} aria-describedby="setup-meeting-window" onChange={(event) => selectTestMeeting(event.target.value)}><option value="">Choose a meeting</option>{setupMeetings.map((meeting) => <option key={meeting.id} value={meeting.id}>{meeting.title}</option>)}</select></label>
    <p id="setup-meeting-window" className="field-help">Scans are accepted from the meeting start until its attendance window closes. To test now, choose today's date and a start time at or before now; the default new meeting starts later.{selectedMeeting?.attendanceClosesAt && <> Selected window: {new Date(selectedMeeting.startsAt).toLocaleString()} to {new Date(selectedMeeting.attendanceClosesAt).toLocaleString()}.</>}</p>
    <div className="wizard-secondary-actions"><button type="button" onClick={() => setCreatingMeeting(true)}>Create meeting</button></div>
  </div>;
  ...
  return <div className="workspace-shell">
    ...
    <MeetingCreationDialog open={creatingMeeting} meetings={meetings} discordEnabled={false} onClose={() => setCreatingMeeting(false)} onCreated={meetingCreated} />
