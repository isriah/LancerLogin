...
async function checkForUpdate(): Promise<Check> {
  ...
  if (!releaseResponse.ok) throw new Error("Release check unavailable"); const release = await releaseResponse.json() as { tag_name?: string }; const latest = release.tag_name?.replace(/^v/, "") ?? ""; const current = installation.releaseVersion.replace(/^v/, ""); const result = { current, latest, available: Boolean(latest && isNewerRelease(latest, current)) }; localStorage.setItem(cacheKey, JSON.stringify({ ...result, checkedAt: Date.now() })); return result;
}
...
export function UpdateIndicator({ openUpdates }: { openUpdates: () => void }) {
  ...
  return <button className="update-indicator" type="button" onClick={openUpdates} aria-label={`LancerLogin ${check.latest} is available. Open Updates.`}><span aria-hidden="true">↑</span> Update available <strong>v{check.latest}</strong></button>;
}
