...
export function UpdatesPage() {
  ...
  return <section className="settings-page settings-updates" aria-labelledby="updates-title"><div className="panel-heading updates-header"><div className="page-intro"><h1 id="updates-title">Updates</h1><p>Compare installed releases and use the guarded backup-first update workflows.</p></div><button className="quiet-button" type="button" aria-describedby="release-check-status" disabled={checkingRelease || retrying} onClick={() => void release.check(true).catch(() => undefined)}>{checkingRelease ? "Checking for updates…" : "Check for updates"}</button></div><p id="release-check-status" className="field-help" role="status">{release.checkedAt ? `Last successful check ${new Date(release.checkedAt).toLocaleString()}.` : "No successful release check yet."}{release.error && release.attemptedAt ? ` Last failed attempt ${new Date(release.attemptedAt).toLocaleString()}.` : ""}{latest && !release.fresh ? " Previously checked release; confirmation is required before updating." : ""}{retrying ? ` Next check after ${new Date(release.retryAt!).toLocaleString()}.` : ""}</p><p className="ui-status settings-notice" data-tone={updateTone} role="status">{installationError || notice || releaseNotice}</p><section className="release-comparisons" aria-label="Release comparisons"><WebUpdateCard current={current} available={dashboardUpdateAvailable} workflowUrl={workflowUrl} latestTag={latest?.tag_name} releaseUrl={latest?.html_url} />{kiosk && <article><div className="panel-heading"><div><h2>Physical kiosk</h2><p>{kiosk.name}</p></div><button className="primary-button" type="button" disabled={kioskBusy || !kioskOnline || !kioskUpdateAvailable} onClick={() => void updateKiosk()}>{kioskBusy ? "Queueing…" : "Update to latest stable"}</button></div><div className="version-grid"><div><span>Installed</span><strong>{kiosk.releaseVersion ?? "Unknown"}</strong></div><div><span>Latest compatible</span><strong>{latest?.tag_name ? formatVersion(latest.tag_name) : "Unavailable"}</strong>{!kioskOnline && <small>The kiosk must be online to update.</small>}</div></div><p className="kiosk-update-status ui-status" data-tone={kioskTone} role="status">{kioskUpdate.message}</p></article>}</section></section>;
}
