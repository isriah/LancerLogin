...
const commandLabel = (command?: Command, kiosk?: Kiosk, latest?: Release) => {
  if (!command) return "No kiosk update has been requested.";
  if (!command.completedAt) return `Queued ${new Date(command.createdAt).toLocaleTimeString()}. Waiting for the kiosk to receive the request.`;
  if (!command.success) return command.resultMessage ? `Kiosk update failed: ${command.resultMessage}` : "Kiosk update failed before the installer started.";
  if (latest?.tag_name && kiosk?.releaseVersion && !isNewerRelease(latest.tag_name, kiosk.releaseVersion)) return `Installed successfully. This kiosk now reports ${formatVersion(kiosk.releaseVersion)}.`;
  return "Kiosk received the request and started its verified installer. Waiting for the restarted service to report its installed version.";
};
...
export function UpdatesPage() {
  ...
  const updateTone = latest ? isNewerRelease(latest.tag_name ?? "", current) ? "warning" : "success" : loading ? "neutral" : "error";
  const kioskTone = latestCommand?.completedAt && latestCommand.success === 0 ? "error" : latestCommand ? "warning" : "neutral";
  return <section className="settings-page settings-updates" aria-labelledby="updates-title"><div className="page-intro"><h1 id="updates-title">Updates</h1><p>Compare installed releases and use the guarded backup-first update workflows.</p></div><p className="ui-status settings-notice" data-tone={updateTone} role="status">{notice}</p><section className="release-comparisons" aria-label="Release comparisons"><article><div className="panel-heading"><div><h2>Dashboard</h2><p>Cloudflare dashboard installation</p></div><button className="primary-button" type="button" disabled={busy || !workflowUrl} onClick={() => void begin()}>{busy ? "Preparing backup…" : "Back up and begin update"}</button></div><div className="version-grid"><div><span>Installed</span><strong>{current}</strong></div><div><span>Latest release</span><strong>{latest?.tag_name ? formatVersion(latest.tag_name) : "Unavailable"}</strong>{latest?.html_url && <a href={latest.html_url} target="_blank" rel="noreferrer">Read release notes</a>}</div></div><ol className="update-steps"><li>Downloads an entire-installation backup.</li><li>Opens the guarded workflow in your private deployment repository in a new tab.</li><li>You select <strong>upgrade</strong>, keep <strong>Latest stable</strong> selected, and enter your installation slug.</li><li>GitHub validates the exact existing resources before applying migrations or deploying.</li></ol></article>{kiosk && <article><div className="panel-heading"><div><h2>Physical kiosk</h2><p>{kiosk.name}</p></div><button className="primary-button" type="button" disabled={kioskBusy || !kioskOnline || !latest || !isNewerRelease(latest.tag_name ?? "", kiosk.releaseVersion ?? "0.0.0")} onClick={() => void updateKiosk()}>{kioskBusy ? "Queueing…" : "Update to latest stable"}</button></div><div className="version-grid"><div><span>Installed</span><strong>{kiosk.releaseVersion ?? "Unknown"}</strong></div><div><span>Latest compatible</span><strong>{latest?.tag_name ? formatVersion(latest.tag_name) : "Unavailable"}</strong>{!kioskOnline && <small>The kiosk must be online to update.</small>}</div></div><p className="kiosk-update-status ui-status" data-tone={kioskTone} role="status">{commandLabel(latestCommand, kiosk, latest)}</p></article>}</section></section>;
}
