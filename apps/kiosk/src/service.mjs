...
const scanner = createScanner({ scanSensor: async () => sensor ? useSensor(() => sensor.scan()) : Promise.reject(new Error("Reader disabled")), setLed, mappings, queue, loadPairing, flushAttendance, onDisplay: showDisplay, onReader: (online) => { state.readerOnline = online; }, onCloud: (online) => { state.cloudOnline = online; } });
...
const server = createServer(async (request, response) => {
  ...
  if (pathname === "/display-state" && request.method === "GET") { response.end(JSON.stringify({ display, kioskName: state.kioskName, readerOnline: state.readerOnline, cloudOnline: state.cloudOnline, pendingEvents: (await queue.pending()).length })); return; }
...
if (process.env.NODE_ENV !== "test") {
  ...
  void loadPairing().then(() => scanner.start()); heartbeat(); setInterval(heartbeat, 60_000).unref();
}
