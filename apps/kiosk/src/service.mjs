...
import { maintenanceApp, maintenanceHtml, maintenanceStyles } from "./maintenance-ui.mjs";
...
const server = createServer(async (request, response) => {
  ...
  if (pathname === "/maintenance.css" && request.method === "GET") { response.setHeader("content-type", "text/css; charset=utf-8"); response.end(maintenanceStyles); return; }
  ...
  if (pathname === "/display-state" && request.method === "GET") { response.end(JSON.stringify({ display, branding, kioskName: state.kioskName, readerOnline: state.readerOnline, cloudOnline: state.cloudOnline, pendingEvents: (await queue.pending()).length, displayReloadToken })); return; }
  ...
  if (pathname === "/enroll" && request.method === "POST") { scanner.pause(); try { await requireNetworkSession(); if (!sensor) throw new Error("Fingerprint reader is disabled"); const input = await body(request); const memberId = input.memberId?.trim(); const finger = String(input.finger || "unspecified").trim().toLowerCase(); const allowedFingers = ["right index", "left index", "right thumb", "left thumb", "right middle", "left middle", "other"]; const slot = Number(input.slot); if (!memberId || memberId.length > 100 || !allowedFingers.includes(finger) || !Number.isInteger(slot) || slot < 0 || slot > 199) throw new Error("Choose a roster member, finger label, and sensor slot from 0 to 199"); const current = await mappings.read(); if (current[String(slot)] && input.replaceExisting !== true) throw new Error(`Sensor slot ${slot} is already mapped; confirm replacement to continue`); await showDisplay("processing", { message: "Enroll fingerprint", detail: "Follow the reader prompts" }); await useSensor(() => sensor.enroll(slot)); await mappings.replace({ ...current, [String(slot)]: { memberId, finger } }); state.readerOnline = true; await showDisplay("welcome", { message: "Enrollment saved", detail: `Sensor slot ${slot} is ready` }); response.statusCode = 201; response.end(JSON.stringify({ enrolled: true, slot, memberId, finger })); } catch (error) { await showDisplay("rejected", { message: "Enrollment failed", detail: error instanceof Error ? error.message : "Try again" }); response.statusCode = 400; response.end(JSON.stringify({ error: error instanceof Error ? error.message : "Fingerprint enrollment failed" })); } finally { scanner.resume(); } return; }
