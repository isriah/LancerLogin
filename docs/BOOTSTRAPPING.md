...
3. In GitHub, add that token as `CLOUDFLARE_API_TOKEN`, then run **Provision adopter installation** in `create` mode. Type the required `CREATE <slug>` confirmation. The workflow creates an adopter-named Worker, D1 database, Pages dashboard, deployment secret, and initial state, then outputs the Pages URL. Use `resume` with `RESUME <slug>` only after an interrupted run.
