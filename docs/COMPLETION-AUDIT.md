...
No existing attendance installation, Cloudflare resource, Raspberry Pi, credential, database, or repository was used. The community collector was deployed only to a fresh collector-only account, and its guarded workflow verified the live service before the reviewed endpoint was added to adopter configuration. A real adopter Create workflow run and Pi/R503 smoke test still require separate targets explicitly supplied for LancerLogin only.
