...
## ADR-013: One-way Google Calendar ownership and recovery

The modular development architecture supersedes separate client credentials: one Google OAuth client serves both individual staff sign-in and the organizational Calendar/Drive grant. Those purposes retain separate identities and proof flows. Incremental consent uses actual scopes, and failed replacement retains the working connection until explicit verified promotion with local Admin recovery. Legacy distinct clients continue operating only during staged migration; incompatible refresh tokens are never moved between clients. See [GOOGLE-CONNECTION.md](GOOGLE-CONNECTION.md). The timing-only Calendar ownership/retry rules below remain in force.

LancerLogin remains authoritative. A meeting occurrence created while Calendar sync is configured receives one stable mapped Google event containing only a generic LancerLogin identity and its start and end times. Occurrence and future-series time edits update only mapped event times. Hiding a mapped meeting queues deletion of its Google event; restoring it creates a new mapped event rather than attempting to revive the deleted provider event. Google-side edits are never imported, and enabling the integration does not backfill older meetings.

Google failures never block a LancerLogin meeting mutation. Failed creates, updates, and deletes enter a durable, installation-scoped retry queue with bounded backoff and actionable Admin status. Deterministic provider event identifiers and persisted operation state prevent retry duplicates. Disabling Calendar pauses delivery while retaining its authorization and queue; removing it deletes the local authorization and pending mapping state but does not make an unverifiable claim that previously delivered Google events were removed.
