...
Google Calendar uses its own optional Admin-authorized OAuth consent and encrypted credential record, separate from Google dashboard sign-in. Enabling, disabling, rotating, or removing Calendar access never changes the installation's sign-in mode or the Google sign-in integration. Calendar authorization requests only the calendar-list read access needed to select one writable calendar and event access needed to maintain LancerLogin-created events, with offline access for background retries.
