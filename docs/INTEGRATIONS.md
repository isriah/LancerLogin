...
Each posted absence notice opens a public thread for questions. The bot needs **Create Public Threads** permission, and members need **Send Messages in Threads** permission in the attendance channel. The **Contest absence** button still requests a private review. The bot removes its tracked notice after the configured contest window on its next five-minute maintenance pass, even when the optional attendance-channel manager is off. The thread may remain in Discord after its starter notice is removed. The optional manager maintains the pinned kiosk status and attendance guide. The optional anomaly report requires a different text channel in the verified server. Administrators must restrict that channel to intended attendance staff. Provider failures never block kiosk scans or an attendance record.
