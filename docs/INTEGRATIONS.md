...
- Discord supports Operator/Admin member linking, missing-member notices with dashboard-visible contest records and audited resolution, calendar event create-or-update mapping, and one persistent kiosk-status message. Operators can refresh it manually; authenticated heartbeats update it when reader, release, or online state changes, and a five-minute scheduled reconciliation marks a stale kiosk offline. Discord failures never block kiosk heartbeat or attendance delivery.
