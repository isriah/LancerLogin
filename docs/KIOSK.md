...
After extraction, the installer restores `/opt/lancerlogin` to mode `0755` so
the service account can traverse root-owned code even if the archive root had
restrictive permissions. Private state remains `0700`. Archive verification
starts the extracted service with synthetic unpaired state; Linux CI also drops
to a distinct unprivileged account. This is software acceptance, not evidence of
a Pi installation, UART or fingerprint-reader test.
...
The API resolves the official `isriah/LancerLogin` latest release with GitHub's required
application User-Agent. A valid but incompatible latest release permits one fallback
to at most 20 release records, selecting the first published stable `v0.n.n` tag.
Drafts and prereleases are never queued. Malformed, redirected, unavailable or
rate-limited feeds fail without queuing a command; both requests and their body reads
share one four-second deadline. This fixed Pi source is separate from the modular
development cloud updater's trust configuration. Mocked Worker tests verify lookup
and command admission; no Pi update or live provider acceptance is implied.

The Pi-side fixed updater independently applies the same compatible-tag selection,
including the single bounded fallback. Its root-owned selector validates the chosen
release's installer/checksum assets and never substitutes another release when those
assets are missing. Metadata uses a shared four-second deadline and one-megabyte
body cap. Downloads remain fixed official URLs with checksum verification.

The requested version is an observed expectation, not a pinned privileged command.
If the official feed changes between queueing and execution, the Pi may select a
different compatible release; the existing heartbeat comparison reports a mismatch
instead of claiming the requested version succeeded. No URL, tag or command-line
override is accepted. Exact queued-tag installation would need a separately reviewed
API and privileged handoff contract. Tests use synthetic feeds and disposable local
artifacts; successful live-Pi delivery remains unverified.

Development import provenance: original repository commits
`03c4951a7a805a066c6c71395f4efee608652b3e` and
`0bcf519d2fa804018311bd3e9b6588c17b4b1787`, with bounded fallback and failure coverage
strengthened in WU-123.
