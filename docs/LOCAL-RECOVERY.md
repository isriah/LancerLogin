...
2. Create or reuse a Cloudflare API token restricted to exactly one adopter-owned account with Account Settings read and D1 edit access. Export it as `CLOUDFLARE_API_TOKEN` only for this terminal session. Never paste it into LancerLogin or commit it; the tool derives the one account ID at runtime.
