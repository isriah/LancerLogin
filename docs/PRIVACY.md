...
RoboLancers operates the optional community collector. Reports are retained for 30 days, and only designated maintainers can access authenticated aggregates. While telemetry is accepted, Admins can copy the deletion-request reference from Setup → Privacy and email it to robolancers@gmail.com. No service-level agreement is offered. The incident and deletion process is published in `docs/TELEMETRY-GOVERNANCE.md`.

The separately deployable collector is documented in `docs/TELEMETRY-COLLECTOR.md`. It hashes the opaque install ID before storage, accepts at most one row per installation per UTC day, returns only authenticated aggregates, hides metro groups smaller than five installations, and performs scheduled retention deletion. No collector endpoint is configured in the community release until the fresh collector deployment succeeds and its HTTPS URL is reviewed.
