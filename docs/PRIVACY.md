...
The separately operated collector is documented in `docs/TELEMETRY-COLLECTOR.md`. It hashes the opaque install ID before storage, accepts at most one row per installation per UTC day, returns only authenticated aggregates, hides metro groups smaller than five installations, and performs scheduled retention deletion. The reviewed community endpoint is configured in adopter deployments, but it receives nothing unless the first Admin explicitly accepts telemetry.
