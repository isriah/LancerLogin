...
Google Calendar sends meeting titles, notes as event descriptions, and meeting times. Restrict the connected calendar to the intended audience. Discord signed interactions are verified before processing. Discord operations are limited to the configured server and LancerLogin-tracked message or event identifiers. The adopter must restrict any staff-only Discord channel it chooses. LancerLogin does not enumerate, moderate, or delete unrelated Discord content.
