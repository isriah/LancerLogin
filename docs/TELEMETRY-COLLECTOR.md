...
Use a fresh collector-only Cloudflare account and dedicated token. The operator needs Account Settings read, Workers Scripts write, D1 write, and permission to configure Worker rate limits. Use Wrangler 4.36.0 or newer.

The recommended path is the manual **Deploy community telemetry collector** GitHub Action. In the protected `telemetry-production` environment, store the dedicated Cloudflare token as `LANCERLOGIN_TELEMETRY_CLOUDFLARE_API_TOKEN`, an independently generated 48-byte base64url HMAC key as `LANCERLOGIN_TELEMETRY_INSTALL_PEPPER`, and an independently generated 48-byte base64url administrator credential as `LANCERLOGIN_TELEMETRY_ADMIN_TOKEN`. Never display or reuse them. The Create operation refuses an existing database or Worker with the reserved name, applies migrations, verifies `/health`, submits and deletes a synthetic report, and reports the public endpoint. Resume requires the dedicated database to exist and does not rotate existing Worker secrets. Keeping the administrator credential in the protected GitHub environment permits approved maintainers to perform aggregate and verified-deletion operations without exposing it.
