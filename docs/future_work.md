...
Dependencies: ADR-010 selects all preserved eligible history from the member's participation start date and keeps Reports filters local to Reports; product decision still required on whether check-in and check-out deviations are averaged as individual scan values or first combined per meeting, and the displayed unit/rounding rule
