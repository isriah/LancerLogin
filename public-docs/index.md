...
Release docs include annotated, alt-text-described visuals for GitHub template creation, GitHub secrets, Cloudflare token scope, initial dashboard setup, pairing-code generation, kiosk sensor testing, Google OAuth configuration, and optional integrations. Provider setup visuals are sanitized captures of the real interfaces where practical; account labels and identifiers are cropped or redacted, secret-entry fields remain empty, and token reveal screens are never captured. Product-interface illustrations use generated example values only.
