...
Release docs will include annotated, alt-text-described screenshots for GitHub template creation, GitHub secrets, Cloudflare token scope, initial dashboard setup, pairing-code generation, kiosk sensor test, Google OAuth configuration, and each optional integration. Screenshots must use generated example values only, never production accounts, people, URLs, IDs, or tokens.
