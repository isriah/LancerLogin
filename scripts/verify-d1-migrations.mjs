...
try {
  ...
  const output = await runWrangler(["d1", "execute", databaseName, "--local", "--config", configPath, "--persist-to", stateDirectory, "--json", "--command", "SELECT name FROM d1_migrations ORDER BY id; SELECT name, sql FROM sqlite_master WHERE name IN ('organization_settings', 'kiosks', 'kiosk_commands', 'idx_kiosk_commands_pending', 'meetings', 'pairing_codes', 'simulated_kiosk_sessions', 'users', 'idx_one_user_per_member', 'idx_meetings_series', 'attendance_events', 'encrypted_integrations', 'integration_verification_challenges', 'idx_integration_verification_expiry', 'discord_attendance_notifications', 'discord_attendance_recipients') ORDER BY name;"], { json: true });
