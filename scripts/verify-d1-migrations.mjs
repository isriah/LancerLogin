...
try {
  ...
  const output = await runWrangler(["d1", "execute", databaseName, "--local", "--config", configPath, "--persist-to", stateDirectory, "--json", "--command", "SELECT name FROM d1_migrations ORDER BY id; SELECT name, sql FROM sqlite_master WHERE name IN ('installations', 'members', 'organization_settings', 'kiosks', 'kiosk_commands', 'idx_kiosk_commands_pending', 'meetings', 'meeting_templates', 'idx_meeting_templates_installation_name', 'pairing_codes', 'simulated_kiosk_sessions', 'users', 'idx_one_user_per_member', 'idx_meetings_series', 'attendance_events', 'encrypted_integrations', 'integration_verification_challenges', 'idx_integration_verification_expiry', 'discord_attendance_notifications', 'discord_attendance_recipients', 'google_calendar_authorizations', 'google_calendar_event_mappings', 'google_calendar_operations', 'idx_google_calendar_operations_due', 'discord_label_role_mappings', 'discord_label_role_challenges', 'discord_label_role_previews', 'discord_label_role_jobs', 'discord_label_role_job_items', 'idx_discord_label_role_one_active_job') ORDER BY name;"], { json: true });
  ...
  if (!brandingSql.includes("late_scan_minutes") || !brandingSql.includes("logo_backdrop") || !brandingSql.includes("discord_channel_manager_enabled") || !attendanceSql.includes("check_out") || !notificationSql.includes("message_id") || !notificationSql.includes("channel_id") || !notificationSql.includes("expires_at") || !notificationSql.includes("deleted_at") || !recipientSql.includes("discord_user_id") || !recipientSql.includes("message_id)")) throw new Error("Final D1 schema is missing attendance lifecycle or durable Discord channel-manager fields");
  ...
  const installationSql = schema.get("installations") ?? "";
  if (!installationSql.includes("google_enabled") || !installationSql.includes("resend_enabled") || !installationSql.includes("discord_enabled") || !installationSql.includes("google_calendar_enabled")) throw new Error("Final D1 schema is missing explicit integration enablement flags");
  const calendarAuthorizationSql = schema.get("google_calendar_authorizations") ?? ""; const calendarMappingSql = schema.get("google_calendar_event_mappings") ?? ""; const calendarOperationSql = schema.get("google_calendar_operations") ?? ""; const calendarDueIndexSql = schema.get("idx_google_calendar_operations_due") ?? "";
  if (!calendarAuthorizationSql.includes("ciphertext") || !calendarAuthorizationSql.includes("verified_at") || !calendarMappingSql.includes("meeting_id") || !calendarMappingSql.includes("generation") || !calendarOperationSql.includes("next_attempt_at") || !calendarOperationSql.includes("'upsert'") || !calendarDueIndexSql.includes("next_attempt_at")) throw new Error("Final D1 schema is missing encrypted Google Calendar authorization or durable event retry state");
  const kioskCommandsSql = schema.get("kiosk_commands") ?? "";
  if (!kioskCommandsSql.includes("reset_network_pin") || !kioskCommandsSql.includes("requested_release_version") || !kioskCommandsSql.includes("resolution_status") || !(schema.get("idx_kiosk_commands_pending") ?? "").includes("completed_at")) throw new Error("Final D1 schema is missing the authenticated kiosk command queue or durable update outcomes");
  const roleMappingSql = schema.get("discord_label_role_mappings") ?? ""; const roleJobSql = schema.get("discord_label_role_jobs") ?? ""; const roleJobItemSql = schema.get("discord_label_role_job_items") ?? "";
  if (!roleMappingSql.includes("UNIQUE (installation_id, guild_id, role_id)") || !roleJobSql.includes("source_fingerprint") || !roleJobItemSql.includes("discord_user_id") || !(schema.get("idx_discord_label_role_one_active_job") ?? "").includes("running")) throw new Error("Final D1 schema is missing scoped Discord role mappings or durable manual sync state");
