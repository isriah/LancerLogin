...
test("Updates shows local information while the release feed stalls, then degrades and recovers", async ({ page }) => {
  ...
  await page.route("**/admin/releases/latest", async (route) => {
    ...
    await route.fulfill({ json: discovered({ tag_name: "v0.15.0", html_url: "https://github.example.test/releases/v0.15.0" }, await page.evaluate(() => Date.now())) });
  });
...
test("Updates keeps a confirmed kiosk release visible across responsive branded themes", async ({ page }, testInfo) => {
  ...
  await page.route("**/admin/releases/latest", (route) => route.fulfill({ json: discovered({ tag_name: "v0.23.0", html_url: "https://github.example.test/releases/v0.23.0" }) }));
...
test("release checks share15-minute cache across reloads while local status polls, and keyboard manual checks coalesce", async ({ page }) => {
  ...
  await pending[0].fulfill({ json: discovered({ tag_name: tag }, await page.evaluate(() => Date.now())) }); await expect(page.locator(".version-grid").first()).toContainText("0.24.0");
  await expect(check).toBeEnabled(); await expect(page.locator("#release-check-status")).toContainText("Last successful check");
...
test("browser GitHub blocking and the old saved cooldown do not block API release discovery", async ({ page }) => {
  let githubReads = 0; let discoveries = 0;
