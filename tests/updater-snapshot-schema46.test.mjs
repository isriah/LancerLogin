...
import { snapshotCatalog } from '../apps/updater/src/snapshot-catalog-46.mjs';

test('reviewed schema46 capture and isolated replay preserve types, operational tables and catalog boundaries', async t => {
  ...
  const snapshotId = crypto.randomUUID(), authority = { ...pins, operationId: snapshotId, jobId: crypto.randomUUID(), epoch: 1, state: 'closed', schema: 46, ledger: snapshotCatalog.ledger };
  const capture = createSnapshotCapture({ pins, database: source, store, catalogKey: 'schema-46-v1', withClosedEpoch: async (_, run) => run(authority) });
  let captured; for (let i = 0; i < 400; i++) { captured = await capture.advance(snapshotId); if (captured.outcome === 'snapshot-candidate') break; }
  assert.equal(captured.outcome, 'snapshot-candidate'); assert.equal(captured.candidate.verified, false); assert.equal(captured.candidate.schema, 46);
  async function fixture(name, customCapture = capture) {
    ...
    const replay = createSnapshotReplay({ pins: { ...pins, validationDatabaseId: crypto.randomUUID() }, database: wrapper, store, capture: customCapture, capability, catalogKey: 'schema-46-v1' });
  ...
  await t.test('full schema, FK order, rowids, int64, REAL, text/blob and sequence survive lost batch acknowledgment', async () => {
    ...
    const result = await finish(f); assert.equal(result.outcome, 'validated'); assert.equal(result.receipt.format, 'typed-replay-v1'); assert.equal(result.receipt.verified, undefined); assert.equal(result.receipt.schema, 46);
  ...
  await t.test('catalog selection rejects wrong live schema, persisted identity and replay manifest', async () => {
    assert.throws(() => createSnapshotCapture({ pins, database: source, store, withClosedEpoch: async () => {}, catalogKey: '../snapshot-catalog-46.mjs' }), /snapshot-catalog-key/);
    const defaultCapture = createSnapshotCapture({ pins, database: source, store, withClosedEpoch: async (_, run) => run(authority) });
    assert.equal((await defaultCapture.advance(crypto.randomUUID())).outcome, 'unknown');
    await assert.rejects(defaultCapture.describe(snapshotId), /snapshot-conflict|snapshot-incomplete/);
    const db = await mf.getD1Database('TAMPER'), capability = {};
    const defaultReplay = createSnapshotReplay({ pins: { ...pins, validationDatabaseId: crypto.randomUUID() }, database: db, store, capture, capability });
    assert.equal((await defaultReplay.advance(capability, { operationId: crypto.randomUUID(), snapshotId })).reason, 'validation-manifest');
    assert.equal((await db.prepare("SELECT count(*) AS n FROM sqlite_schema WHERE name NOT GLOB 'sqlite_*' AND name NOT GLOB '_cf_*'").first()).n, 0);
    const manifest = await capture.describe(snapshotId);
    const wrongManifest = { ...manifest, chainSha256: '0'.repeat(64) };
    const f = await fixture('EXTRA', { describe: async () => wrongManifest });
    assert.equal((await f.advance()).reason, 'validation-manifest'); assert.equal(f.batches(), 0);
  });
